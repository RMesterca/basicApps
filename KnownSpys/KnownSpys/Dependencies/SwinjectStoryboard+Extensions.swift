...
extension SwinjectStoryboard {
    public class func setup() {
        if AppDelegate.dependencyRegistry == nil {
            AppDelegate.dependencyRegistry = DependencyRegistryImplementation(container: defaultContainer)
        }
        ...
        func main() {
            dependencyRegistry.container.storyboardInitCompleted(SpyListViewController.self) { r, vc in
                ...
                vc.configure(with: presenter, navigationCoordinator: coordinator, spyCellMaker: dependencyRegistry.makeSpyCell)
            }
        }
